...
import { DerivedTask, Priority, Status } from "@/types";
...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  const revenueByStatus = ["Todo", "InProgress", "Done"].map((s) => ({
    status: s as Status,
    revenue: tasks
      .filter((t) => t.status === s)
      .reduce((s2, t) => s2 + (t.revenue ?? 0), 0),
  }));
  // Injected bug: assume numeric ROI across the board; mis-bucket null/NaN
  const roiBuckets = [
    {
      label: "<200",
      count: tasks.filter((t) => t.roi != null && t.roi < 200).length,
    },
    {
      label: "200-500",
      count: tasks.filter((t) => t.roi != null && t.roi >= 200 && t.roi <= 500)
        .length,
    },
    {
      label: ">500",
      count: tasks.filter((t) => t.roi != null && t.roi > 500).length,
    },
